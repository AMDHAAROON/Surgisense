...
type HistoryItem = { at: Date; tools: DetectedTool[] };

export default function Home() {
  const [history,   setHistory]   = useState<HistoryItem[]>([]);
  const [lastTools, setLastTools] = useState<DetectedTool[]>([]);

  const handleDetection = useCallback((tools: DetectedTool[]) => {
    setLastTools(tools);
    if (tools.length > 0) {
      setHistory(prev => {
        const next = [...prev, { at: new Date(), tools }];
  ...
  const connected = true; // always "connected" — detection via POST
