...
import {
  Camera, Upload, ScanSearch, Save, RotateCcw,
  CheckCircle2, XCircle, AlertTriangle, ShieldCheck,
  Package, ArrowRightLeft, Layers3
} from "lucide-react";

// ─── Types ────────────────────────────────────────────────────────────────────
interface BBox { x1: number; y1: number; x2: number; y2: number; }
interface InventoryTool { name: string; confidence: number; box: BBox; }
interface ReconcileResult {
  allPresent: boolean; preCount: number; postCount: number;
  present: InventoryTool[]; missing: { name: string }[];
  extra: InventoryTool[]; summary: string;
}
...
// ─── BBox SVG Overlay ─────────────────────────────────────────────────────────
function BBoxOverlay({ tools, missingNames, imgW, imgH }: {
  tools: InventoryTool[]; missingNames: string[]; imgW: number; imgH: number;
}) {
  return (
    <svg
    ...
    >
      {tools.map((tool, i) => {
        ...
        const px = x1 * imgW, py = y1 * imgH;
        const pw = (x2 - x1) * imgW, ph = (y2 - y1) * imgH;
        ...
        return (
          <g key={i}>
            ...
            <rect x={px} y={Math.max(0, py - 22)} width={txt.length * 7.5 + 12} height={20}
              fill={color} rx={3} />
            <text x={px + 6} y={Math.max(14, py - 7)}
              fill="white" fontSize={12} fontFamily="system-ui, sans-serif" fontWeight="bold">
              {txt}
            </text>
          </g>
        );
...
export default function Inventory() {
  ...
  const [status,       setStatus]       = useState<{ msg: string; type: "ok" | "err" | "warn" | "info" } | null>(null);
  ...
  const startCamera = useCallback(async () => {
    ...
    } catch { setMsg("Could not access camera.", "err"); }
  }, []);
  ...
  const captureFrame = useCallback(() => {
    ...
    setCapturedImg(url); setTools([]); setReconcile(null);
  ...
  const scanImage = useCallback(async () => {
    ...
    } catch { setMsg("Scan failed. Check backend is running.", "err"); }
  ...
  const savePreSurgery = useCallback(async () => {
    ...
    try {
      ...
      const data = await res.json();
      setPreSessionId(data.id); setPreSaved(true);
      setMsg(`Pre-surgery inventory saved — ${tools.length} tool(s) recorded.`, "ok");
    } catch { setMsg("Failed to save inventory.", "err"); }
  ...
  const reconcilePost = useCallback(async () => {
    ...
    } catch { setMsg("Reconciliation failed.", "err"); }
    finally { setSaving(false); }
  }, [capturedImg, tools, preSessionId]);

  const reset = () => {
    setCapturedImg(null); setTools([]); setReconcile(null); setStatus(null); setStreaming(false);
  };
  ...
  return (
    <div className="space-y-10">
      ...
      <section className="relative text-center max-w-3xl mx-auto space-y-5 pt-4">
        ...
        <h1 className="text-5xl font-black tracking-tight bg-clip-text text-transparent bg-gradient-to-b from-foreground to-foreground/60 leading-[1.1]">
          Surgical <span className="text-primary italic">Inventory</span>
        </h1>
        <p className="text-muted-foreground text-lg leading-relaxed">
          Photograph the instrument tray before and after surgery. AI detects every tool and flags anything missing.
        </p>
      </section>
      ...
      <div className="flex justify-center">
        <div className="flex gap-2 p-1 bg-muted/30 rounded-full border border-border/50">
          {([["pre", "Before Surgery", Package], ["post", "After Surgery", ArrowRightLeft]] as const).map(
            ([p, label, Icon]) => (
              <button
                key={p}
                onClick={() => { setPhase(p); reset(); }}
                className={cn(
                  "relative flex items-center gap-2 px-6 py-2.5 text-sm font-semibold transition-all rounded-full overflow-hidden",
              ...
              >
                {phase === p && (
                  <div className={cn(
                    ...
                    p === "pre" ? "bg-primary shadow-primary/30" : "bg-violet-600 shadow-violet-600/30"
                  )} />
                )}
      ...
      {phase === "idle" && (
        <div className="glass-card rounded-2xl p-16 text-center text-muted-foreground border border-border/30">
          ...
          <p className="text-sm mt-1 opacity-60">Capture before surgery to build inventory, then after to reconcile</p>
        </div>
      )}
          ...
          <div className="lg:col-span-8 space-y-4">
            <div className="glass-card overflow-hidden rounded-2xl">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="font-bold text-lg">
                      {phase === "pre" ? "Pre-Surgery Capture" : "Post-Surgery Capture"}
                    </h2>
              ...
              <div className="relative bg-black" style={{ aspectRatio: "4/3" }}>
                ...
                {!streaming && capturedImg && (
                  <div className="relative w-full h-full">
                    <img src={capturedImg} alt="Tray scan"
                    ...
                    {tools.length > 0 && (
                      <BBoxOverlay tools={tools} missingNames={missingNames}
                ...
                {scanning && (
                  <div className="absolute inset-0 bg-black/60 backdrop-blur-sm flex flex-col items-center justify-center gap-4">
                    ...
                    <p className="text-white text-sm font-medium">Analysing with Groq Vision…</p>
                  </div>
                )}
              </div>
              ...
              <div className="p-5 bg-muted/5 border-t border-white/10 flex flex-wrap gap-3">
                ...
                {/* Webcam flow */}
                {!streaming && !capturedImg && (
                  <Button onClick={startCamera} className="gap-2 rounded-full font-bold">
                    <Camera className="h-4 w-4" /> Start Webcam
                  </Button>
                )}
                {streaming && (
                  <Button onClick={captureFrame} className="gap-2 rounded-full font-bold bg-green-600 hover:bg-green-700">
                ...
                {capturedImg && !scanning && tools.length === 0 && (
                  <Button onClick={scanImage} className="gap-2 rounded-full font-bold">
                ...
                {capturedImg && tools.length > 0 && (
                  <Button variant="outline" onClick={scanImage} disabled={scanning}
                    className="gap-2 rounded-full font-bold border-2">
                ...
                {capturedImg && tools.length > 0 && phase === "pre" && !preSaved && (
                  <Button onClick={savePreSurgery} disabled={saving}
                ...
                {capturedImg && tools.length > 0 && phase === "post" && !reconcile && (
                  <Button onClick={reconcilePost} disabled={saving}
          ...
          <div className="lg:col-span-4 space-y-4">
                  ...
                  {tools.map((t, i) => {
                    ...
                    return (
                      <div key={i} className={cn(
                        ...
                        <span className="text-xs font-black" style={{ color: confColor(t.confidence) }}>
            ...
            {reconcile && (
              <div className={cn(
                ...
                reconcile.allPresent ? "border-green-500/30" : "border-red-500/30"
              )}>
                <div className={cn(
                  ...
                  <h3 className={cn("font-bold", reconcile.allPresent ? "text-green-400" : "text-red-400")}>
                    {reconcile.allPresent ? "All Tools Accounted For" : "Missing Tools Found"}
                  </h3>
                </div>
                ...
                <div className="p-4 space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {[["Before", reconcile.preCount], ["After", reconcile.postCount]].map(([label, count]) => (
                  ...
                  {reconcile.missing.length > 0 && (
                    <div>
                      ...
                      {reconcile.missing.map((m, i) => (
                        <div key={i} className="flex items-center gap-2 px-3 py-2 rounded-lg bg-red-500/10 border border-red-500/20 mb-1.5">
                          <XCircle className="h-3.5 w-3.5 text-red-400 shrink-0" />
                          <span className="text-sm text-red-400 font-medium">{toolLabel(m.name)}</span>
                        </div>
                      ))}
                    </div>
                  )}
                  ...
                  {reconcile.extra.length > 0 && (
                    <div>
                      ...
                      {reconcile.extra.map((t, i) => (
                        <div key={i} className="flex items-center gap-2 px-3 py-2 rounded-lg bg-amber-500/10 border border-amber-500/20 mb-1.5">
                          <AlertTriangle className="h-3.5 w-3.5 text-amber-400 shrink-0" />
                          <span className="text-sm text-amber-400 font-medium">{toolLabel(t.name)}</span>
                        </div>
                      ))}
                    </div>
                  )}
            ...
            {preSaved && phase === "pre" && (
              <div className="glass-card rounded-2xl p-5 border border-green-500/20 bg-green-500/5">
                ...
                <p className="text-sm text-muted-foreground">
                  Switch to <strong className="text-foreground">After Surgery</strong> tab when the procedure is complete.
                </p>
              </div>
            )}
            ...
            {tools.length > 0 && (
              <div className="glass-card rounded-2xl p-4 space-y-2">
                <p className="text-xs font-black uppercase tracking-widest text-muted-foreground mb-3">Legend</p>
                {[
